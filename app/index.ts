...
import { existsSync, mkdirSync, rmSync, readFileSync, readdirSync, copyFileSync, writeFile, writeFileSync } from "fs";
...
class DB {
    ...
    constructor(path: string = './index.json', options: Config = {
        ...
        if (existsSync(this.path)) { this.JSON(JSON.parse(readFileSync(this.path, "utf-8"))); } else { writeFileSync(this.path, '[]'); }
    ...
    set(val: any, id = randomBytes(5).toString('base64url'),) {
